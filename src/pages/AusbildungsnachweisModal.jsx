...
export default function AusbildungsnachweisModal({ onClose }) {
  ...
  const [mailStatus, setMailStatus] = useState(null) // null | 'sending' | 'ok' | string(error)
    ...
    try {
      ...
      const { data, error } = await supabase.functions.invoke('resend-email', {
        body: {
          ...
          titel: `Ausbildungsnachweis ${form.datum || ''}`,
        },
      })
      ...
      setMailStatus(err.message || 'PDF-Generierung fehlgeschlagen')
      setTimeout(() => setMailStatus(null), 6000)
    }
  }
  ...
  return (
    <div className="modal-backdrop" onClick={e => e.target === e.currentTarget && onClose()}>
      <div className="modal" style={{ maxWidth: 600, maxHeight: '90vh', overflowY: 'auto' }}>
        ...
        <div style={{ display: 'flex', gap: 8, justifyContent: 'flex-end', marginTop: 8, paddingTop: 16, borderTop: '1px solid var(--gray-100)', flexWrap: 'wrap' }}>
          ...
          <button className="btn btn-primary" onClick={drucken} disabled={generating}>
            {generating ? 'Wird erstellt...' : '🖨️ Lokal drucken / PDF'}
          </button>
        </div>
      </div>
    </div>
