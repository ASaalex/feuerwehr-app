...
function PruefungErstellen({ profile, onBack, importDaten }) {
  ...
  async function handleSave() {
    ...
    if (pruefung) {
      await supabase.from('fragen').insert(
        fragen.map(({ id, ...f }, i) => ({ ...f, pruefung_id: pruefung.id, reihenfolge: i }))
      )
    }
    setSaving(false)
    onBack()
    ...
    <div>
      <div className="page-header">
        ...
        <button className="btn btn-primary" onClick={handleSave} disabled={saving}>
          {saving ? 'Speichern...' : 'Pruefung speichern'}
        </button>
      </div>

      {istImport && (
        <div className="alert alert-success" style={{ marginBottom: 16 }}>
          {fragen.length} Fragen aus JSON importiert — bitte pruefen und bei Bedarf anpassen.
        </div>
      )}
      ...
      <div className="card" style={{ marginBottom: 20 }}>
        ...
        <div className="form-group">
          <label>Titel der Pruefung</label>
          <input value={form.titel} onChange={e => setForm(f => ({ ...f, titel: e.target.value }))} placeholder="z.B. Atemschutzgeraetetraeger Grundausbildung" />
        </div>
        <div className="form-group">
        ...
        <div className="form-group">
          ...
          <input type="number" min="1" max="100" value={form.bestehens_prozent}
            onChange={e => setForm(f => ({ ...f, bestehens_prozent: parseInt(e.target.value) }))} style={{ maxWidth: 100 }} />
        </div>
      </div>
...
function PruefungAblegen({ pruefung, profile, onBack }) {
  ...
  async function handleAbgabe() {
    ...
    await supabase.from('pruefungs_ergebnisse').upsert({
      kamerad_id: profile.id, pruefung_id: pruefung.id,
      punkte_erreicht: richtig, punkte_gesamt: gesamt, bestanden, antworten_detail: antworten
    })
    setErgebnis(res)
    setSubmitted(true)
  }
  ...
  if (submitted && ergebnis) return (
    <div>
      <div className="page-header">
        <h1>Ergebnis</h1>
        <button className="btn btn-secondary" onClick={onBack}>Zurueck</button>
      </div>
  ...
  return (
    <div>
      ...
      <div style={{ marginTop: 16 }}>
        <button className="btn btn-primary btn-lg" style={{ width: '100%', justifyContent: 'center' }} onClick={handleAbgabe}>
          Pruefung abgeben
        </button>
      </div>
    </div>
...
function PruefungAuswertung({ pruefung, onBack }) {
  ...
  const durchschnitt = ergebnisse.length
    ? Math.round(ergebnisse.reduce((s, e) => s + (e.punkte_gesamt > 0 ? e.punkte_erreicht / e.punkte_gesamt * 100 : 0), 0) / ergebnisse.length)
    : 0
...
function PruefungBearbeiten({ pruefung, profile, onBack }) {
  ...
  async function handleSave() {
    ...
    if (fragen.length > 0) {
      await supabase.from('fragen').insert(
        fragen.map(({ id, isNeu, ...f }, i) => ({ ...f, pruefung_id: pruefung.id, reihenfolge: i }))
      )
    }
    setSaving(false)
    onBack()
  ...
  return (
    <div>
      ...
      {fragen.length === 0 && (
        <div className="card" style={{ textAlign: 'center', color: 'var(--gray-400)', padding: 32 }}>
          <p>Keine Fragen. Klicke auf "Frage hinzufuegen".</p>
        </div>
      )}
      {fragen.map((frage, fi) => (
        <div key={frage.id} className="card" style={{ marginBottom: 12 }}>
